...
\begin{theorem}
Toda matriz es equivalente por filas a una \alert{única} matriz
...
Veamos la unicidad. Sean \(R\) y \(S\) dos matrices escalonadas y
reducidas equivalentes por filas.

Denotemos por \(R_i\) y \(S_i\) sus filas. Como las operaciones
elementales por filas no alteran las relaciones lineales entre las
columnas, para cada \(j\) la columna \(j\) es combinación lineal de las
columnas anteriores en \(R\) si y solo si lo es en \(S\).

Por tanto, \(R\) y \(S\) tienen sus pivotes en las mismas columnas.
...
j_1<j_2<\cdots<j_r
\]
las columnas de los pivotes.

Como \(R\) y \(S\) son reducidas, las submatrices formadas por estas
columnas son, en ambos casos,
\[
\begin{pmatrix}
I_r\\
0
...
Para cualquier otra columna \(j\), sus entradas en las primeras \(r\)
filas son precisamente los coeficientes de su expresión como
combinación lineal de las columnas pivote.

Como estas relaciones lineales son las mismas en \(R\) y en \(S\),
la columna \(j\) coincide en ambas matrices.

Por tanto, todas las columnas de \(R\) y \(S\) coinciden, y
\[
R=S.
\]
\end{proof}
