...
\[
\xrightarrow{\substack{\frac12F_1\\ \frac12F_2}}
\begin{pmatrix}
1&2&1&0&4\\
0&0&1&\frac12&-\frac12\\
0&0&0&2&-5\\
...
\begin{frame}[allowframebreaks]{De la forma escalonada a la forma reducida}
Partiendo de la forma escalonada calculada anteriormente:
\[
\begin{pmatrix}
1&2&1&0&4\\
0&0&1&\frac12&-\frac12\\
...
\begin{frame}[allowframebreaks]{Matrices elementales y operaciones por filas}

\begin{proposition}
Sean \(A\in M_{m\times n}(k)\) y \(E\in M_m(k)\) una matriz
elemental. Entonces \(EA\) es la matriz que se obtiene de \(A\)
efectuando la misma operación elemental por filas que transforma
\(I_m\) en \(E\).
\end{proposition}

\begin{proof}
Escribamos las filas de \(A\) por bloques:
\[
A=
\begin{pmatrix}
A_1\\
...
Consideramos los tres tipos de operaciones elementales.

\medskip

\textbf{Tipo 1.}
Si \(E\) se obtiene de \(I_m\) intercambiando las filas \(i\) y \(j\),
entonces
\[
EA=
\begin{pmatrix}
...
\end{pmatrix}.
\]
Por tanto, \(EA\) se obtiene de \(A\) mediante
\(F_i\leftrightarrow F_j\).

\medskip

\textbf{Tipo 2.}
Si \(E\) se obtiene de \(I_m\) mediante \(\lambda F_i\),
con \(\lambda\neq0\), entonces
\[
EA=
\begin{pmatrix}
...
\end{pmatrix}.
\]
Por tanto, \(EA\) se obtiene de \(A\) mediante \(\lambda F_i\).

\medskip

\textbf{Tipo 3.}
Si \(E\) se obtiene de \(I_m\) mediante \(F_i+\lambda F_j\),
con \(i\neq j\), entonces
\[
EA=
\begin{pmatrix}
...
En los tres casos, la multiplicación por \(E\) a la izquierda realiza
sobre \(A\) la correspondiente operación elemental por filas.
\end{proof}
